...
public class myOval extends myShape{
    ...
    public void draw(GraphicsContext gc,double width, double height) {
        ...
        // Draw circle
        gc.strokeOval(super.getX() - width/2,super.getY()- height/2,2*getAxisA(),2*getAxisB());
        gc.fillOval(super.getX() - width/2,super.getY()- height/2,2*getAxisA(),2*getAxisB());
    }
    ...
    public myRectangle getBoundingBox(double canvasWidth, double canvasHeight,Color color1){
        myRectangle rectangle1 = new myRectangle(getX() ,getY() ,color1,2*getAxisA(),2*getAxisB());
    ...
    public double getPerimeter(){
        return 2*Math.PI*Math.sqrt((Math.pow(getAxisA(),2)+Math.pow(getAxisA(),2))/2);
    }
